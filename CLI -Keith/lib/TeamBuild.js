...
// The TeamBuild constructor is responsible for assembling the team and controlling the assembly of the team.  
...
class TeamBuild {
  ...
  makeGuess() {
    this.askForLetter().then(() => {
      // If the user has no guesses remaining after this guess, show them the word, ask if they want to assemble again
  ...
  // Asks the user if they want to assemble again after running out of guessesLeft
  askToassembleAgain() {
    inquirer
      .prompt([
        {
          type: "confirm",
          name: "choice",
          message: "assemble Again?"
        }
      ])
      .then(val => {
        // If the user says yes to another TeamBuild, assemble again, otherwise quit the TeamBuild
        if (val.choice) {
          this.assemble();
        } else {
          this.quit();
